...
echo "=== setup wallet: farmmonkeywallet ==="
# key for eosio account and export the generated password to a file for unlocking wallet later
cleos wallet create -n farmmonkeywallet --to-console | tail -1 | sed -e 's/^"//' -e 's/"$//' > farmmonkey_wallet_password.txt
# Owner key for farmmonkeywallet wallet
cleos wallet import -n farmmonkeywallet --private-key 5JpWT4ehouB2FF9aCfdfnZ5AwbQbTtHBAwebRXt94FmjyhXwL4K
# Active key for farmmonkeywallet wallet
cleos wallet import -n farmmonkeywallet --private-key 5JD9AGTuTeD5BXZwGQ5AtwBqHK21aHmYnTetHgk1B3pjj7krT8N

# * Replace "farmmonkeywallet" with your own wallet name when you start your own project
